...
const Clearance = ({open, onClose}) => {
    if(!open) return null;
    const apply = async() => {
        const endpointUrl = `https://dbproject-group22.herokuapp.com/student/clearance`;
        const options = {
            method: "GET",
        };
        ...
        <>
            ...
            <div style={{position: 'fixed', top: '50%', left: '50%', transform:'translate(-50%, -50%)', backgroundColor: 'transparent', padding: '20px', zIndex: '1000', borderRadius: '10px', display: 'flex', flexDirection: 'column'}}>
                <div style={{width: '500px', background: `white`, borderRadius: '2rem', display: 'flex', flexDirection: 'column'}}>
                    <span style={{width: '100%', fontSize: '1.5rem', display: 'flex', justifyContent: 'center', padding: '1rem', background: '#073b4c', borderTopLeftRadius: '2rem', borderTopRightRadius: '2rem', color: '#fff', fontWeight: 'bold'}}>Graduation Clearance</span>
                    <span style={{width: '100%'}}>
                    <span style={{fontSize: '1.2rem', padding: '1rem', height: '100%', display: 'flex', textAlign: 'center', marginTop: '1rem', paddingTop: '0px'}}><b style={{color: '#073b4c'}}>Note: </b>Only those students who are seniors can apply for graduation clearance.</span>
